...
### 2.5 BYOK 与 operator-blind(待用户定 A/B)
- **A(实用)**:上游 key 存 GCP KMS/Secret Manager,IAM 条件绑 `image_digest`,只有度量值正确的 enclave 能取。残留:项目 owner 可改 IAM。
- **B(最强)**:sealed BYOK —— 客户验完 enclave 拿公钥,把 key **封给该度量值的 enclave**,控制面只存密文,enclave RAM 解封。残留:重启需重封。
- 现状:key 经 `tee-env` 注入(operator-visible),是过渡。**这块与"前端 BYOK 录入"是同一件事。**
...
- 数据直连绕过 New API → 用**旁路**把回执回传:push webhook / pull API / 客户端回传 / 批量对账(可组合)。
- **回执可验签** → 合作方(及我们)能确认计量真来自被度量 enclave,**连我们都改不了这个数**。既计费又对账(防抵赖)。
...
## 6. 现状(2026-08-03)与路线图

**Live**:enclave `34.21.247.73:9090`(fidrouter,distroless,`ed7aadcd…`)· 验证页 `https://verify.136-85-124-88.sslip.io`(现场绿灯)· 开源 `github.com/aoraki-labs/fidrouter`。已通:真 TEE + 度量值覆盖代码 + attested E2EE + 能力令牌 + 签名回执 + BYOK 真 Claude + OpenAI 兼容端点 + drop-in SDK + 内容级可复现 + 中立验证页/回执验证。

**路线图**
- **P1**:operator-blind(A/B 定)· 计量回执 webhook 外发 + 用户 id 透传 · 合作方前端(注册入口 + BYOK 录入 + 用量看板)。
